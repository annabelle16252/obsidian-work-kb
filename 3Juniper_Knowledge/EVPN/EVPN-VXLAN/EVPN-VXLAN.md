...
# Underlay&Overlay
- Underlay provides IP reachability between VTEPs, usually through a spine-leaf IP fabric. (VXLAN tunnels are logically between VTEPs, which are usually leaf nodes)
- Overlay uses BGP EVPN as the control plane and VXLAN as the data plane to carry tenant traffic between VTEPs.

- VXLAN does not require a tunnel setup protocol. As long as the underlay provides IP reachability between VTEPs, VXLAN traffic can be encapsulated and forwarded.
- EVPN tells VTEPs how to reach remote MAC addresses, IP addresses, or IP prefixes, and which remote VTEP those destinations are associated with.

# Route Type #
**Type 1: Ethernet Auto-Discovery route**
This route is mainly used for multihoming. It advertises Ethernet Segment information so remote PEs know that a CE is attached to more than one PE.
It also supports fast convergence functions such as aliasing and mass withdrawal. 
In all-active multihoming, split-horizon behavior is used to prevent loops, so BUM traffic received from a multihomed segment is not sent back toward that same segment.

**Type 2: MAC/IP Advertisement route**
This route advertises host(attached to CE side) reachability information.
A Type 2 route can carry:
MAC address only
MAC and IP binding together
It is used so remote PEs can learn where a MAC, or a MAC-IP pair, is located. 
MAC withdrawal is also handled through EVPN control-plane signaling rather than just simple flooding behavior.

**Type 3: Inclusive Multicast Ethernet Tag route**
This route is used for BUM traffic handling. Tell local VTEP which remote VTEPs are members of the same L2 VNI, so it knows where to replicate BUM traffic.
It advertises VTEP reachability for ingress replication or multicast delivery, so VTEPs know where to send broadcast, unknown unicast, and multicast traffic for a given L2 VNI. It is not really "used to establish a tunnel" in the control-plane sense, but rather to advertise BUM replication membership.

**Type 4: Ethernet Segment route**
This route is used between PEs attached to the same Ethernet Segment. 
It helps remote PEs identify the multihomed segment and is involved in Designated Forwarder election procedures. In short, it supports Ethernet Segment discovery and DF election for multihoming.

**Type 5: IP Prefix route(prefix routes inside a VRF)**
This route advertises pure IP prefix information in EVPN. It is mainly used for inter-subnet routing and L3 VNI services. It is not about "leaf-to-spine VXLAN tunnel establishment". In a typical VXLAN EVPN fabric, Type 5 routes advertise tenant IP prefixes between VTEPs, while spines usually act as route reflectors rather than VXLAN tunnel endpoints.

# VNI
- L2 VNI is used to extend a Layer 2 segment across the VXLAN fabric.
- L3 VNI is used to provide Layer 3 routing for a VRF across the VXLAN fabric.
```
Tenant-A
├─ VRF-A
│  └─ L3 VNI 50000
...
Host A (VLAN 10)
  -> ingress on Leaf 1 via L2 VNI for VLAN 10
  -> routed by IRB/default gateway in the VRF
  -> forwarded across the fabric in the L3 VNI
  -> remote Leaf delivers traffic toward VLAN 20
  -> Host B (VLAN 20)
```

# DF
多归（Multi-homing）+ 同一 ESI的时候需要选一个DF.
![[Pasted image 20260413081302.png]]

# ESI
**CE-PE两侧都必须配 LACP，它 在 ESI AE 场景里的作用机制**
PE 先通过 EVPN ESI 选出 active/standby
PE 再通过 LACP 告诉 CE 哪条 member 该转发
CE 的 ae只把流量发到 LACP 允许的 member
这样数据面才和 EVPN 控制面一致
# Command
**看一个MAC/IP是从哪个remote PE学到的
```
> show route table VBASE-21.evpn.0 evpn-mac-address cc:e1:7f:0b:bf:c0
...
101.234.3.134是rmote vtep ip
第二个比第一个多了192.168.1.11，这个地址是source CE的接口地址
```

**看local设备的EVPN MAC表**
```
> show evpn mac-table extensive cc:e1:7f:0b:bf:c0
